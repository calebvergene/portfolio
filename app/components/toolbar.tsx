...
const Toolbar = () => {
  ...
  // Scroll handler to smoothly scroll to the corresponding section
    ...
    >
      <div className="flex row justify-center space-x-3 font-Epilogue text-sm">
        {buttonItems.map((item, index) => (
          <motion.button
            key={item.name}
            className="bg-transparent p-2 py-2 px-4 text-md flex items-center justify-center rounded-lg hover:py-1 hover:pt-1.5 hover:mt-0.5 hover:bg-gray-500/55 hover:mb-1 duration-500 text-white/90"
            ...
            onClick={() => handleScroll(item.link)} // Call the scroll handler on click
          >
            {item.name}
          </motion.button>
        ))}
